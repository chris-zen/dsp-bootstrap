...
pub fn audio_start(pa: &PortAudio) -> Result<PortAudioStream, Error> {
  println!("PortAudio:");
  ...
  let settings = DuplexStreamSettings::new(input_params, output_params, SAMPLE_RATE, FRAMES_PER_BUFFER);
  ...
  let callback = move |DuplexStreamCallbackArgs {
    ...
    dsp::process(in_buffer, out_buffer, frames);

    portaudio::Continue // portaudio::Complete to stop the audio
  };
